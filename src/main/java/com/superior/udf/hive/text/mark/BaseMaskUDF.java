...
package com.superior.udf.hive.text.mark;
...
class DateTransformerAdapter extends AbstractTransformerAdapter {
  ...
  @Override
  public Object getTransformedWritable(DeferredObject object) throws HiveException {
    java.sql.Date value = columnType.getPrimitiveJavaObject(object.get());

    if(value != null) {
      Date transformedValue = transformer.transform(Date.ofEpochMilli(value.getTime()));
